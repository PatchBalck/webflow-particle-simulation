      ...
      // ===== AUDIO SETUP =====
      const audio = new Audio('/examples/audio/Boombox-audio.mp3');
      let isPlaying = false;
      ...
      loader.load(
        '/examples/models/Boombox-01.glb',
        (gltf) => {
          boombox = gltf.scene;
...
<style>
  ...
  @media (max-width: 1024px), (hover: none) {
    ...
    .cursor-button {
      ...
      font-size: clamp(0.9rem, 1vw + 0.3rem, 1rem);
      font-weight: 400;
      width: clamp(4.5rem, 1vw + 5rem, 20rem);
      height: clamp(4.5rem, 1vw + 5rem, 20rem);
      border-radius: 50%;
      display: flex;
      align-items: center;
      justify-content: center;
  ...
  .letter-wrapper.animate .letter:first-child {
    transform: translateY(140%) skewY(-7deg);
  }

  .letter-wrapper.animate .letter + .letter {
